...
1. MCP `@aios/mcp` — tools `aios_*` (`aios_contract_version`, `aios_operational_state`, `aios_governance_*`, `aios_audit_docs`, `aios_memory_*`, `aios_provider_health`, `aios_provider_models`, `aios_provider_chat`, `aios_run_pipeline`, `aios_run_across_workspaces`, `aios_compile_prompt`, `aios_list_workspaces`, `aios_workspace_*`, `aios_build_knowledge`, …)
